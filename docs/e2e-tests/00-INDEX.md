...
| 04 | `04-portal-customer.md` | `portal.sentrikat.com` customer: OTP login, dashboard, account, licenze, downloads, support/feedback, checkout, upgrade, logout | ✅ re-test 7-dim 2026-05-06 (4 finding cross-repo: 1 MEDIUM beta-LATEST mitigato manualmente da operatore, 1 MEDIUM Unknown CVE count, 1 INFO Enterprise cap, 1 INFO Company empty; OTP + branding ✅ post sentrikat-web PR #256+#257) |
| 05 | `05-admin-portal.md` | `portal.sentrikat.com` admin: 25 pagine admin (customers, licenses, plans, leads, demo-requests, feedback, audit, logs, usage-metrics, ecc.) | 🟡 24/25 pagine mappate; **8/10 HIGH chiusi 2026-05-06** post PR #252-#263 (releases popolate, datasources nominati, RSA_PRIVATE_KEY configurato, CSP fix, newsletter 403, EA Tenants stats, Centralized Logs, Last Login); 1 declassato HIGH→MEDIUM (`[05.21.1]` /admin/plans hardcoded ma admin-only); 1 ancora aperto (`[05.4.1]` /admin/status vs /admin/datasources mismatch); 3 nuovi (`[05.1.2]` CVE Findings KPI, `[05.1.3]` NaN MB, `[05.3.2]` CISA KEV+EUVD ops issue) → bundlati in PR #264 DRAFT sentrikat-web Week 1 (≥2026-05-09) |
| 06 | `06-app-auth-rbac.md` | `app.sentrikat.com` auth: local, LDAP/AD, SAML 2.0, TOTP 2FA, session, password reset, RBAC (super_admin/org_admin/manager/user) | 🟡 16 aree mappate (auth completo + RBAC + LDAP+SAML+TOTP+sessione+lockout+audit), walkthrough live richiede testlab Keycloak+OpenLDAP |
| 07 | `07-agents-inventory.md` | Agent: download script Win/Linux/macOS, deploy, API key, inventory, heartbeat, job processing, asset mgmt, container scan, dependency scan (sentrikat-scan CLI) | 🟡 20 aree mappate (lifecycle + ingestion + asset mgmt); walkthrough live richiede docker compose locale + Win VM testlab |
| 08 | `08-scanning-matching.md` | Vuln matching: CISA KEV sync, NVD/CVE.org/ENISA fallback, CPE 4-tier mapping, 3-phase filter (derivatives/history-guard/noise), backport detection (OSV/RedHat/MSRC/Debian), EPSS | 🟡 18 aree mappate, **08.1 CISA KEV sync chiusa** 2026-05-06 (4 finding: 1 MEDIUM scheduler 06/05 02:00 FAILED, 1 MEDIUM UX footer non-live, 1 INFO path /admin-panel#settings:system, 1 INFO declassato falso allarme); 5 critical-path test pytest scritti come regression guard per F.2/F.6/F.8/matching invariant |
...
| 13 | `13-admin-ops.md` | Admin panel core, super admin, health checks, logs, Prometheus metrics, OpenTelemetry, backup/restore (on-prem), scheduler jobs, maintenance, cleanup | 🟡 15 aree mappate; **13.1 admin landing chiusa** 2026-05-06 (sidebar diff on-prem vs SaaS coerente; 5 finding: `[13.1.1]` HIGH pagination "of 0", `[13.1.2]` MED filter placeholder, `[13.1.3]` MED last sync FAILED, `[13.1.4]` MED SaaS SMTP not configured, `[13.1.5]` INFO no agent reports atteso) |
| 14 | `14-saas-specific.md` | SaaS: quota/limiti per piano, feature gating, isolamento multi-tenant, license webhook, trial expiry, upgrade/cancel, metering, addons | 🟡 22 aree mappate (multi-tenancy + plans + metering + provisioning); walkthrough live richiede SaaS access + sentrikat-web bridge |
| 15 | `15-security-edge.md` | SQL injection, XSS, CSRF, LDAP injection, SSRF, command injection, path traversal, rate limiting, lockout, encryption at rest (Fernet), edge cases (DB down, disk full, NVD down, ecc.) | 🟡 40 aree mappate (OWASP A01-A07 + edge cases); OWASP smoke statico già fatto (0 reali findings, vedi `docs/audits/owasp-sample-audit-2026-05-06.md`); pentest professionale ~5-10k€ raccomandato pre-customer-enterprise |
...
| **2026-04-29** | 03 | **✅ VERIFY ROUND 2 — on-prem core fix sidebar** (PC casa, docker rebuild beta.6 fresh, branch main `a4d035f`) | Container `sentrikat` ricostruito con `docker compose build && up --force-recreate`, `cat VERSION` → `1.0.0-beta.6` (era `1.0.0-beta.2`).<br>✅ **[03.11.2.3]** sidebar Users & Access mostra `LDAP Users` + `LDAP Groups` come super_admin in DEMO on-prem (screenshot allegato).<br>✅ **[03.6.6]/[03.7.2]/[03.7.4]** sezione "Platform Operations" NON visibile nel sidebar on-prem.<br>✅ Route inesistenti danno HTTP 404 (testato `/superadmin` → "404 - Page Not Found").<br>✅ **Sub-test esplicito**: `curl http://localhost/super-admin/webhook-events` → `HTTP/1.1 404 NOT FOUND` + `curl http://localhost/super-admin/usage-uploads` → `HTTP/1.1 404 NOT FOUND`. Fix Flask `_require_super_admin()` con `abort(404)` su `is_saas_mode()=False` confermato funzionante.<br>🆕 **NEW BUG `[03.15.1]`** la 404 page è plain HTML senza styling SentriKat (probabilmente template Flask di default, manca `errorhandler(404)` con render_template del layout). Severity 🔵 INFO — UX minore.<br>🆕 **NEW BUG `[03.15.2]`** HTTPS rotto in locale: `curl -v -k https://localhost/` → `Recv failure: Connection was aborted` durante TLS handshake (anche su root, non solo super-admin). Browser HSTS upgrade automatico su path `super-admin` confonde test browser-based. Solo HTTP funziona localmente. Severity 🟡 MEDIUM — non blocking per dev, ma in prod HTTPS è obbligatorio. Da indagare cert config in `nginx/ssl/` / docker entrypoint. |
| **2026-04-29** | 03 | **🔧 FIX [03.11.4.5] — design escalation, ALLOW_PRIVATE_URLS rispettato in on-prem prod** (commit `44a94b1`) | **Problema**: il fix originale di `[03.11.4.5]` era cosmetico (log noise + UI hint) e marcato "by-design", ma il design era SBAGLIATO per la modalità on-prem. La policy SSRF in `app/network_security.py::_allow_private_urls()` ignorava `ALLOW_PRIVATE_URLS=true` in `FLASK_ENV=production` indipendentemente dalla modalità deployment. **Conseguenza per il cliente on-prem**: Jira/GitLab/Webhook/Syslog interni alla LAN (IP RFC1918) non testabili in production → integrations inutilizzabili nel caso d'uso primario.<br>**🔧 Fix applicato**: `_allow_private_urls()` ora bypassa il blocco se EITHER `FLASK_ENV=development` OR `is_saas_mode()=False`. In SaaS production il flag resta ignorato (è lì che SSRF verso metadata cloud o tenant fratelli è la minaccia reale). Aggiornato anche `private_urls_ignored_in_production()` e docstring di `is_ssrf_safe_url()`.<br>**Verify**: ✅ **VERIFIED 2026-04-29 PC casa**. Container ricreato con il fix, `FLASK_ENV=production` `ALLOW_PRIVATE_URLS=true` `SAAS_MODE=unset`, Test Connection Jira → `http://host.docker.internal:8080` con project key `VULN` → toast successo, nessun `SSRF blocked` nei log.<br>**Sbloccati**: ✅ 03.11.4 (Jira), 03.11.5 (Webhook), 03.11.6.4 (GitLab), 03.11.6.8 (YouTrack), 03.11.2.9 (LDAP login E2E indirettamente). Test funzionali ora eseguibili in production mode. |
| **2026-04-29** | 03 | **🟢 Fase 03.14 (sezione 2) — click reali Sync/Alert triggers (Strategia A) HIGH coverage chiusa** | **Nota**: i numeri `[03.14.1]`–`[03.14.22]` erano già usati nella sezione originale, questa batteria parte da `[03.14.23]`.<br>✅ **[03.14.24]** Sync CISA KEV Now → toast verde<br>✅ **[03.14.25]** Sync EPSS Scores Now → toast verde<br>✅ **[03.14.26]** Sync CPE Dictionary Now → toast verde<br>✅ **[03.14.27]** Run Auto-Acknowledge Now → toast verde<br>✅ **[03.14.28]** Send Email Alerts Now → 0 delivery (skip by-design "no unacknowledged CVEs") MA code path validato senza SSRF block<br>✅ **[03.14.29]** Send Webhook Alerts Now → idem, skip by-design<br>✅✅ **[03.14.30]** verify definitivo `[03.11.4.5]` su Webhook (Test individuale → `host.docker.internal:8800` → toast verde + request in webhook-tester)<br>🔵 **[03.14.23]** `/admin-panel` redirige a `/all-users` (non bug)<br>🔵 **[03.14.31]** UI permette `localhost:<port>` come Webhook URL ma fallisce a runtime senza hint Docker<br>**Test follow-up rinviati**: Strategia F seed CVE per validare delivery vera + 7-dim dim 5/6/7 sui sync triggers. |
| **2026-04-29** | 03 | **🔧 FIX [03.14.34] — progress tracking DB-backed cross-worker** (commit `a0830cd`) | Scoperto durante test 03.14: dopo "Sync CISA KEV Now" il client riceveva 404 ricorrenti su `/api/progress/<job_id>` perché `app/progress.py` era un dict in-memory module-level e gunicorn ha 3-5 worker, ognuno con il suo dict.<br>**🔧 Fix**: riscritto `progress.py` come DB-backed via `system_settings` (key=`progress:<id>`, category=`progress`). Tutti i worker leggono dallo stesso storage Postgres → cross-worker visibility garantita. Niente Redis (evitata nuova dipendenza in deploy on-prem), niente migration (riusa schema esistente).<br>**✅ VERIFIED**: console F12 silente dopo rebuild, polling riceve `200 OK` con JSON `{status, step, percent}`, `psql` mostra la riga aggiornata in tempo reale.<br>**Effetto**: probabile sblocco di `[03.14.32]` (0 match dopo sync) — il client ora riceve il terminal status e aggiorna la UI. Da rifare match test post-sync.<br>**Bug bonus aperti durante test**: `[03.14.32]` 🟡 match 0 (sospetto fix [03.14.34] lo risolve), `[03.14.33]` 🔵 a11y aria-hidden Bootstrap modal, `[03.14.35]` 🔵 bottone Sync cliccabile dopo refresh, `[03.14.34.1]` ⏸️ pollProgress senza max-retries. |
...
## Appendix — E2E Flows Index (legacy)
