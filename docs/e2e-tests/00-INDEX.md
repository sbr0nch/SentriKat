...
| **2026-04-29** | 03 | **🟢 Fase 03.14 — click reali Sync/Alert triggers (Strategia A) HIGH coverage chiusa** | Eseguiti i 6+ click che il master segnava come "HIGH — senza questi la feature non è validata".<br>✅ **[03.14.1]** Sync CISA KEV Now → toast verde<br>✅ **[03.14.2]** Sync EPSS Scores Now → toast verde<br>✅ **[03.14.3]** Sync CPE Dictionary Now → toast verde<br>✅ **[03.14.4]** Run Auto-Acknowledge Now → toast verde<br>✅ **[03.14.5]** Send Email Alerts Now → 0 delivery (skip by-design "no unacknowledged CVEs") MA code path validato senza SSRF block<br>✅ **[03.14.6]** Send Webhook Alerts Now → idem, skip by-design<br>✅✅ **[03.14.7]** verify definitivo `[03.11.4.5]` su Webhook (Test individuale → `host.docker.internal:8800` → toast verde + request in webhook-tester)<br>🔵 **[03.14.0]** info: `/admin-panel` redirige a `/all-users` (non bug)<br>🔵 **[03.14.8]** info: UI permette `localhost:<port>` come Webhook URL ma fallisce a runtime senza hint Docker<br>**Test follow-up rinviati**: Strategia F seed CVE per validare delivery vera + 7-dim dim 5/6/7 sui sync triggers. |
