...
- **Discovered**: 2026-04-23
- **🔧 Fix 2026-05-01** (commit pending): chiuso il bypass su entrambi i path SSO. (1) `app/saml_manager.py` `get_or_create_saml_user()`: prima di creare un nuovo User row, chiama `check_user_limit()` di `app/licensing.py`. Se `allowed=False` ritorna `(None, False, 'license_limit')` invece di creare. **Signature change**: la funzione ora ritorna 3-tupla `(user, created, error_code)` (era 2-tupla); error_code può essere `'no_email' | 'no_org' | 'license_limit' | None`. (2) `app/saml_api.py` `saml_acs()`: caller adattato al triple, distingue redirect `error=saml_license_limit` da `error=saml_user_create_failed` per messaggio user-friendly. (3) `app/templates/login.html`: aggiunta entry `saml_license_limit` → "User limit for this license has been reached. Contact your administrator to upgrade or remove an inactive user before logging in via SSO." (4) `app/ldap_sync.py` `auto_provision_user_on_login()`: stesso check prima di `db.session.add(user)`, ritorna dict con `success=False` + `error` originale dal license_info. **Path utenti esistenti** (saml-type, ldap-type, local trovato per email canonical) **NON tocca** il limit — solo nuove creazioni triggherano il check. Verifica pending: Community Edition con 1/1 users → SAML login da Keycloak con email nuova → login bloccato + redirect login con messaggio chiaro (era login successful + 2/1 users silenzioso).
