...
console.log('Admin Panel JS Loading... VERSION 20250121f - TIMESTAMP:', new Date().toISOString());
                    ...
                    <tbody>
                        ${pageResults.map((user, index) => {
                            ...
                            const actionButton = user.exists_in_db
                                ...
                                : `<button class="btn btn-sm btn-primary" onclick="showInviteLdapUserModalInline(${userIndex})">
 ...
 */
async function showInviteLdapUserModalInline(userIndex) {
    console.log('showInviteLdapUserModalInline called with index:', userIndex);

    const user = ldapSearchCache.results[userIndex];
    if (!user) {
