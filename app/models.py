...
class SharedView(db.Model):
    ...
    description = db.Column(db.Text, nullable=True)
    filters = db.Column(db.Text, nullable=False)  # JSON string of filters
    ...
    # Relationships
    creator = db.relationship('User', backref='shared_views')
    organization = db.relationship('Organization', backref='shared_views')
