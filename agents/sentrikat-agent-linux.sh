    ...
    if [[ "${SCAN_EXTENSIONS:-false}" == "true" ]]; then
        ...
        for user_home in /home/* /root; do
            ...
            for chrome_dir in "$user_home/.config/google-chrome" "$user_home/.config/chromium"; do
                [[ ! -d "$chrome_dir" || -L "$chrome_dir" ]] && continue
            ...
            for ff_profile in "$user_home"/.mozilla/firefox/*.default*; do
                ...
                # Parse extensions.json - each addon has id, version, defaultLocale.name
...
" "$ff_ext_json" 2>/dev/null) || true
            ...
            # --- Edge extensions ---
            local edge_dir="$user_home/.config/microsoft-edge"
            ...
            for jb_dir in "$user_home"/.config/JetBrains/*/; do
        ...
        if command -v go &>/dev/null; then
            for search_dir in /home /opt /srv /var/www; do
                ...
                while IFS= read -r gosumfile; do
                    ...
                    while IFS= read -r line; do
                        # Skip lines with /go.mod suffix (checksum entries)
                        ...
                        # Strip v prefix and /go.mod suffix
